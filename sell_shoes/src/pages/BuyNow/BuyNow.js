...
function Cart() {
    ...
    const payArr = ['Thanh toán tiền mặt', 'Thanh toán VNPay'];
    const handlePay = (pay, index) => {
        setActive(index);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.log(error);
            toast.error('Vui lòng kiểm tra kết nối của bạn và thử lại');
        }
    };
    useEffect(() => {
                                ...
                                <div className={cx('body-products')}>
                                    <div className={cx('body-product')}>
                                        <img
                                            ...
                                            alt="Hinh anh"
                                            src={product.id_product.image || product.image}
                                        ></img>
                                        <p className={cx('body-product_name')}>{product.id_product.name_product}</p>
                                    </div>
                                </div>
                            </div>
            ...
            <div className={cx('pay')}>
                ...
                <div className={cx('button-pay')}>
                    {payArr.map((pay, index) => (
                        <div
                            onClick={() => handlePay(pay, index)}
                            key={index}
                            className={cx('button-submit', { active: active === index })}
                        >
                            {pay}
                        </div>
                    ))}
